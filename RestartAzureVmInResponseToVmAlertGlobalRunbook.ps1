...
.DESCRIPTION
    ...
    Input is alert data with information needed to identify which VM to restart.
...
if ($WebhookData) 
{
    ...
    if (($status -eq "Activated") -or ($status -eq "Fired"))
    {
        ...
        if ($ResourceType -eq "Microsoft.Compute/virtualMachines")
        {
            ...
            # Restart the ARM VM
            Write-Verbose "Restarting the VM - $ResourceName - in resource group - $ResourceGroupName -" -Verbose
            Restart-AzVM -Name $ResourceName -ResourceGroupName $ResourceGroupName -DefaultProfile $AzureContext
            # [OutputType("PSAzureOperationResponse")]
        }
        else {
